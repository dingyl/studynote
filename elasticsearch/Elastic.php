...
class Elastic
{
    ...
    /**
     * 条件查询
     * @param array $cond['age' => ['gt'=>10,'lt'=>20]]
     * @param int $page
     * @param int $per_page
     * @return array
     */
    public function find($cond = [],$page=1,$per_page=10)
    {
        $condition = isset($cond['conditions']) ? $cond['conditions'] : [];
        isset($cond['order']) && $this->order = $cond['order'];
        $this->limit($per_page,($page-1)*$per_page);
        $params = [
            'index' => $this->index,
            'type' => $this->type,
            'from' => $this->offset,
    ...
    public function findAll($cond=[]){
        return $this->find($cond,1,PHP_INT_MAX);
    }
